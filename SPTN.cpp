	...
	vector<int> intervalRunTime, vector<int> runSequence, vector<vector <int> > queueLine, vector<int> queueLineName)	//구간 별 실행 시간, 실행 순서, 대기열(대기열은 push만하고 pop 하지 말아주세요 ! 기록용 대기열입니다)
{
	...
	while (exitProcessCnt < processes) {
		...
		if (runProcess != -1) {
			...
			queue.insert(queue.begin(), runProcess);
			runProcess = -1;
		}
		...
		//잔여 BurstTime Queue 복사
		queueBurstTime.clear();
		for (int queueSwaper = 0; queueSwaper < queue.size(); queueSwaper++) queueBurstTime.push_back(0);
		...
		for (int queueSwaper = 0; queueSwaper < queue.size(); queueSwaper++) {
			for (int queueInSwaper = queueSwaper + 1; queueInSwaper < queue.size(); queueInSwaper++) {
				if (queueBurstTime[queueSwaper] > queueBurstTime[queueInSwaper]) {
					int btemp;
					int qtemp;
					qtemp = queueBurstTime[queueSwaper];
					queueBurstTime[queueSwaper] = queueBurstTime[queueInSwaper];
					queueBurstTime[queueInSwaper] = qtemp;

					btemp = queue[queueSwaper];
					queue[queueSwaper] = queue[queueInSwaper];
					queue[queueInSwaper] = btemp;
				}
			}
		}
		//프로세서 할당
		//대기열에 누군가 있는가 ? 그리고 실행중인 프로세스가 없는가 ?
