...
//call by reference
					//입력 변수
void reffrenceTimes(const int processes, int _arrivalTime[], const int *burstTime, const int timeQuantum,	
					//반환 변수
	int *waitingTime, int *turnAroundTime, double *normalizedTurnAroundTime,								
					//시각화 데이터
	vector<int> intervalRunTime, vector<int> runSequence, vector<vector <int> > queueLine, vector<int> queueLineName)	
{
